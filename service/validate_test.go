...
func Test_ValidateTier2Request(t *testing.T) {
	...
	tests := []struct {
		...
		{"negative start block num", func(req *pbssinternal.ProcessRangeRequest) {
			req.Modules = nil
		}, fmt.Errorf("validate tier2 request: no modules found in request")},
		{"completely below first streamable block", func(req *pbssinternal.ProcessRangeRequest) {
			req.FirstStreamableBlock = 110
		}, fmt.Errorf("validate tier2 request: segment is completely below the first streamable block")},
		{"half below first streamable block", func(req *pbssinternal.ProcessRangeRequest) {
			req.FirstStreamableBlock = 109
		}, nil},
		{"working", func(req *pbssinternal.ProcessRangeRequest) {}, nil},
	}
